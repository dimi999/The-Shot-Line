      ...
      grupRandare.push(React.createElement("div",{key:"d"+i, className:"panouIngredient"},[
        ...
        React.createElement(ButonCantitate, {
          ...
          React.createElement(ButonCantitate, {
            semn:'', 
            key:"reset"+i, 
            ingredient:{cantitate:"0", unitate:""},
            tip:this.props.tip,
            numeIngredient:numeIngredient,
            onClick:this.props.onClick}),
...
class GrupIngrediente extends React.Component {
  ...
  resetCantitate() {
    console.log(this.vectIngrediente)
    for(let tip in this.vectIngrediente) {
      for(let produs in this.vectIngrediente[tip]) {
        this.vectIngrediente[tip][produs]= {cant:0, calorii:0};
      }
    }
    this.totalCalorii=0;
    this.setState({});
  ...
  render(){
    ...
    let mesajAfis = mesaje["peste limita"];
    for(let key in mesaje)
      if(this.totalCalorii <= key && key != "peste limita") {
        mesajAfis = mesaje[key];
        break;
      }
        
    rez.push(React.createElement("p",null,`${mesajAfis}`))
    rez.push(React.createElement("button", {onClick:this.resetCantitate}, 'Reseteaza'))
  return rez;
  }
}
